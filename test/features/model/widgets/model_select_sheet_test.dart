...
void main() {
  ...
  test(
    'single model selection updates assistant when per-chat is off',
    () async {
      const selected = ChatModelTarget(
        providerKey: 'provider-0',
        modelId: 'provider-0-model-01',
      );
      var assistantUpdates = 0;
      var conversationUpdates = 0;

      await persistSingleChatModelSelection(
        perChatModelEnabled: false,
        selected: selected,
        updateAssistant: (target) async {
          assistantUpdates += 1;
          expect(target, selected);
        },
        updateConversation: (target) async {
          conversationUpdates += 1;
        },
      );

      expect(assistantUpdates, 1);
      expect(conversationUpdates, 0);
    },
  );

  test(
    'single model selection updates conversation when per-chat is on',
    () async {
      const selected = ChatModelTarget(
        providerKey: 'provider-0',
        modelId: 'provider-0-model-01',
      ...
      expect(assistantUpdates, 0);
      expect(conversationUpdates, 1);
    },
  );
