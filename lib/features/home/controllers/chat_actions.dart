...
class ChatActions {
  ...
  Future<bool> _completeBatchIfReady(
    ...
    if (_activeAssistantMessages.hasActiveConversation(conversationId)) {
      return false;
    }
  ...
  Future<void> _finishPreparingMessage(
    ...
    final finalized = message.copyWith(isStreaming: false);
    ...
    chatController.setGenerationState(message.id, GenerationRunState.failed);
    chatController.updateMessageInList(message.id, finalized);
    onMessagesChanged?.call();
    await _completeBatchIfReady(conversationId, fallback: finalized);
  }
  ...
  Future<ChatActionResult> sendMessage({
    ...
    if (chatController.appendPersistedTailMessages(<ChatMessage>[
      userMessage,
  ...
  Future<void> cancelStreaming(Conversation? conversation) async {
    ...
    _activeAssistantMessages.removeIfMatches(streaming);
    ...
    await _completeBatchIfReady(cid, fallback: latestStreaming);
  }
  ...
  Future<void> _executeGeneration(stream_ctrl.GenerationContext ctx) async {
    ...
    if (!_activeAssistantMessages.isActive(ctx.assistantMessage)) {
      _activeAssistantMessages.put(ctx.assistantMessage);
      _markBatchStarted(
        conversationId,
        shouldGenerateTitle: ctx.generateTitleOnFinish,
      );
      chatController.setGenerationState(
        state.messageId,
        GenerationRunState.preparing,
  ...
  Future<void> _finishStreaming(
    ...
    _activeAssistantMessages.removeIfMatches(finalizedMessage);
    _batchHadSuccessfulReply[conversationId] = true;
    chatController.setGenerationState(messageId, GenerationRunState.completed);
    if (shouldGenerateTitle) {
      _batchShouldGenerateTitle[conversationId] = true;
    }
  ...
  Future<void> _handleStreamError(
    ...
    final errorMessage = state.ctx.assistantMessage.copyWith(
      content: displayContent,
      isStreaming: false,
      totalTokens: state.totalTokens,
    );
    _activeAssistantMessages.removeIfMatches(errorMessage);
    _batchHadFailure[conversationId] = true;
    chatController.setGenerationState(messageId, GenerationRunState.failed);
